...
public class Valeur {
    ...
    /**
     * Méthode qui permet de trouver la liste de noeuds correspondants au chemin menant au noeud passé en paramètre
     *
     * @param destination noeud jusqu'auquel on veut remonter
     * @return une liste de noms
     */
    public List<String> calculerChemin(String destination){
        //créé la liste de nom à renvoyer
        List<String> res = new ArrayList<String>();
        res.add(0, destination);
        //prend le nom Courant
        String nomCourant = getParent(destination);

        // ajoute les noms dans le début de la liste jusqu'à ce qu'un élément n'ait plus de parent
        while(nomCourant != null){
            res.add(0, nomCourant);
            nomCourant = getParent(nomCourant);
        }
